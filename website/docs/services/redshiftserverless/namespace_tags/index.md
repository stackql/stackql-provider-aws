...
Expands all tag keys and values for <code>namespaces</code> in a region
...
## `SELECT` examples
Expands tags for all <code>namespaces</code> in a region.
```sql
SELECT
region,
...
tag_key,
tag_value
FROM aws.redshiftserverless.namespace_tags
...
For permissions required to operate on the <code>namespace_tags</code> resource, see <a href="/services/redshiftserverless/namespaces/#permissions"><code>namespaces</code></a>
