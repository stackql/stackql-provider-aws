...
Expands all tag keys and values for <code>fargate_profiles</code> in a region
...
## `SELECT` examples
Expands tags for all <code>fargate_profiles</code> in a region.
```sql
SELECT
region,
cluster_name,
...
For permissions required to operate on the <code>fargate_profile_tags</code> resource, see <a href="/services/eks/fargate_profiles/#permissions"><code>fargate_profiles</code></a>
