...
Expands all tag keys and values for <code>hypervisors</code> in a region
...
## `SELECT` examples
Expands tags for all <code>hypervisors</code> in a region.
```sql
SELECT
region,
host,
...
For permissions required to operate on the <code>hypervisor_tags</code> resource, see <a href="/services/backupgateway/hypervisors/#permissions"><code>hypervisors</code></a>
