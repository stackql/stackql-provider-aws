...
Expands all tag keys and values for <code>rooms</code> in a region
...
## `SELECT` examples
Expands tags for all <code>rooms</code> in a region.
```sql
SELECT
region,
arn,
...
For permissions required to operate on the <code>room_tags</code> resource, see <a href="/services/ivschat/rooms/#permissions"><code>rooms</code></a>
