...
Expands all tag keys and values for <code>trust_anchors</code> in a region
...
## `SELECT` examples
Expands tags for all <code>trust_anchors</code> in a region.
```sql
SELECT
region,
enabled,
...
For permissions required to operate on the <code>trust_anchor_tags</code> resource, see <a href="/services/rolesanywhere/trust_anchors/#permissions"><code>trust_anchors</code></a>
