...
Expands all tag keys and values for <code>matching_workflows</code> in a region
...
## `SELECT` examples
Expands tags for all <code>matching_workflows</code> in a region.
```sql
SELECT
region,
workflow_name,
...
For permissions required to operate on the <code>matching_workflow_tags</code> resource, see <a href="/services/entityresolution/matching_workflows/#permissions"><code>matching_workflows</code></a>
