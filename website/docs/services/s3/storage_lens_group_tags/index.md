...
Expands all tag keys and values for <code>storage_lens_groups</code> in a region
...
## `SELECT` examples
Expands tags for all <code>storage_lens_groups</code> in a region.
```sql
SELECT
region,
name,
...
tag_key,
tag_value
FROM aws.s3.storage_lens_group_tags
...
For permissions required to operate on the <code>storage_lens_group_tags</code> resource, see <a href="/services/s3/storage_lens_groups/#permissions"><code>storage_lens_groups</code></a>
