...
Expands all tag keys and values for <code>replicators</code> in a region
...
## `SELECT` examples
Expands tags for all <code>replicators</code> in a region.
```sql
SELECT
region,
replicator_arn,
...
For permissions required to operate on the <code>replicator_tags</code> resource, see <a href="/services/msk/replicators/#permissions"><code>replicators</code></a>
