...
Expands all tag keys and values for <code>multiplexes</code> in a region
...
## `SELECT` examples
Expands tags for all <code>multiplexes</code> in a region.
```sql
SELECT
region,
arn,
...
For permissions required to operate on the <code>multiplex_tags</code> resource, see <a href="/services/medialive/multiplexes/#permissions"><code>multiplexes</code></a>
