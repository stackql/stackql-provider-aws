...
Expands all tag keys and values for <code>site_to_site_vpn_attachments</code> in a region
...
## `SELECT` examples
Expands tags for all <code>site_to_site_vpn_attachments</code> in a region.
```sql
SELECT
region,
core_network_id,
...
tag_key,
tag_value
FROM aws.networkmanager.site_to_site_vpn_attachment_tags
...
For permissions required to operate on the <code>site_to_site_vpn_attachment_tags</code> resource, see <a href="/services/networkmanager/site_to_site_vpn_attachments/#permissions"><code>site_to_site_vpn_attachments</code></a>
