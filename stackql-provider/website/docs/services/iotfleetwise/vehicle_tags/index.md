...
Expands all tag keys and values for <code>vehicles</code> in a region
...
## `SELECT` examples
Expands tags for all <code>vehicles</code> in a region.
```sql
SELECT
region,
arn,
...
For permissions required to operate on the <code>vehicle_tags</code> resource, see <a href="/services/iotfleetwise/vehicles/#permissions"><code>vehicles</code></a>
