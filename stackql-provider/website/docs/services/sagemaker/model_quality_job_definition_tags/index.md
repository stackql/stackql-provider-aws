...
Expands all tag keys and values for <code>model_quality_job_definitions</code> in a region
...
## `SELECT` examples
Expands tags for all <code>model_quality_job_definitions</code> in a region.
```sql
SELECT
region,
job_definition_arn,
...
tag_key,
tag_value
FROM aws.sagemaker.model_quality_job_definition_tags
...
For permissions required to operate on the <code>model_quality_job_definition_tags</code> resource, see <a href="/services/sagemaker/model_quality_job_definitions/#permissions"><code>model_quality_job_definitions</code></a>
