...
Expands all tag keys and values for <code>data_quality_job_definitions</code> in a region
...
## `SELECT` examples
Expands tags for all <code>data_quality_job_definitions</code> in a region.
```sql
SELECT
region,
job_definition_arn,
...
tag_key,
tag_value
FROM aws.sagemaker.data_quality_job_definition_tags
...
For permissions required to operate on the <code>data_quality_job_definition_tags</code> resource, see <a href="/services/sagemaker/data_quality_job_definitions/#permissions"><code>data_quality_job_definitions</code></a>
