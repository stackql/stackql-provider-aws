...
Expands all tag keys and values for <code>rulesets</code> in a region
...
## `SELECT` examples
Expands tags for all <code>rulesets</code> in a region.
```sql
SELECT
region,
name,
...
For permissions required to operate on the <code>ruleset_tags</code> resource, see <a href="/services/databrew/rulesets/#permissions"><code>rulesets</code></a>
