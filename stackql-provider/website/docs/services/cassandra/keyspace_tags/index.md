...
Expands all tag keys and values for <code>keyspaces</code> in a region
...
## `SELECT` examples
Expands tags for all <code>keyspaces</code> in a region.
```sql
SELECT
region,
keyspace_name,
...
For permissions required to operate on the <code>keyspace_tags</code> resource, see <a href="/services/cassandra/keyspaces/#permissions"><code>keyspaces</code></a>
