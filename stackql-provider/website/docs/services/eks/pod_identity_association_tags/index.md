...
Expands all tag keys and values for <code>pod_identity_associations</code> in a region
...
## `SELECT` examples
Expands tags for all <code>pod_identity_associations</code> in a region.
```sql
SELECT
region,
cluster_name,
...
For permissions required to operate on the <code>pod_identity_association_tags</code> resource, see <a href="/services/eks/pod_identity_associations/#permissions"><code>pod_identity_associations</code></a>
