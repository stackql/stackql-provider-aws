...
Expands all tag keys and values for <code>tls_inspection_configurations</code> in a region
...
## `SELECT` examples
Expands tags for all <code>tls_inspection_configurations</code> in a region.
```sql
SELECT
region,
...
tag_key,
tag_value
FROM aws.networkfirewall.tls_inspection_configuration_tags
...
For permissions required to operate on the <code>tls_inspection_configuration_tags</code> resource, see <a href="/services/networkfirewall/tls_inspection_configurations/#permissions"><code>tls_inspection_configurations</code></a>
