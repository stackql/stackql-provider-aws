...
Expands all tag keys and values for <code>ipam_scopes</code> in a region
...
## `SELECT` examples
Expands tags for all <code>ipam_scopes</code> in a region.
```sql
SELECT
region,
ipam_scope_id,
...
For permissions required to operate on the <code>ipam_scope_tags</code> resource, see <a href="/services/ec2/ipam_scopes/#permissions"><code>ipam_scopes</code></a>
