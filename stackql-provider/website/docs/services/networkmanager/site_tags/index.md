...
Expands all tag keys and values for <code>sites</code> in a region
...
## `SELECT` examples
Expands tags for all <code>sites</code> in a region.
```sql
SELECT
region,
site_arn,
...
For permissions required to operate on the <code>site_tags</code> resource, see <a href="/services/networkmanager/sites/#permissions"><code>sites</code></a>
