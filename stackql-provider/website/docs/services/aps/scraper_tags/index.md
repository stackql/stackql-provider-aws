...
Expands all tag keys and values for <code>scrapers</code> in a region
...
## `SELECT` examples
Expands tags for all <code>scrapers</code> in a region.
```sql
SELECT
region,
scraper_id,
...
For permissions required to operate on the <code>scraper_tags</code> resource, see <a href="/services/aps/scrapers/#permissions"><code>scrapers</code></a>
