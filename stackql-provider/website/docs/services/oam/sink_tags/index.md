...
Expands all tag keys and values for <code>sinks</code> in a region
...
## `SELECT` examples
Expands tags for all <code>sinks</code> in a region.
```sql
SELECT
region,
arn,
...
For permissions required to operate on the <code>sink_tags</code> resource, see <a href="/services/oam/sinks/#permissions"><code>sinks</code></a>
