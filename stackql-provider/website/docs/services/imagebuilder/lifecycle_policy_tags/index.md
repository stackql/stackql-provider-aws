...
Expands all tag keys and values for <code>lifecycle_policies</code> in a region
...
## `SELECT` examples
Expands tags for all <code>lifecycle_policies</code> in a region.
```sql
SELECT
region,
arn,
...
For permissions required to operate on the <code>lifecycle_policy_tags</code> resource, see <a href="/services/imagebuilder/lifecycle_policies/#permissions"><code>lifecycle_policies</code></a>
