...
Expands all tag keys and values for <code>route_calculators</code> in a region
...
## `SELECT` examples
Expands tags for all <code>route_calculators</code> in a region.
```sql
SELECT
region,
calculator_arn,
...
For permissions required to operate on the <code>route_calculator_tags</code> resource, see <a href="/services/location/route_calculators/#permissions"><code>route_calculators</code></a>
