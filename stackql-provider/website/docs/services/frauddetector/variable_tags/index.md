...
Expands all tag keys and values for <code>variables</code> in a region
...
## `SELECT` examples
Expands tags for all <code>variables</code> in a region.
```sql
SELECT
region,
name,
...
For permissions required to operate on the <code>variable_tags</code> resource, see <a href="/services/frauddetector/variables/#permissions"><code>variables</code></a>
