...
Expands all tag keys and values for <code>capacity_reservations</code> in a region
...
## `SELECT` examples
Expands tags for all <code>capacity_reservations</code> in a region.
```sql
SELECT
region,
arn,
...
For permissions required to operate on the <code>capacity_reservation_tags</code> resource, see <a href="/services/athena/capacity_reservations/#permissions"><code>capacity_reservations</code></a>
